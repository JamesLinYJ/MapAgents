...
基于短时临近预报（短临）预报 NC 文件和行政区划 shapefile，生成分类风险等级区划图，
包含统计图表和图例。支持边界粗细、文字大小、风险阈值等参数调整。

用法:
    python regional_map.py \
        --nc-file "../数据/短时临近预报（短临）预报产品/202604091955_202604092000.nc" \
...
def main():
    ...
    # 标题
    title = args.title or f"浙江省{args.variable}短时临近预报（短临）预报风险等级区划图"
    if not args.title:
