...
- 需要保留来源归属的维护注记，应记录可确认的日期、实施者或来源，以及简要说明。可使用以下通用模板，并按文件的注释语法调整；注记中的实施者不替换文件的原始作者：
  ```
  维护记录 (YYYY-MM-DD):
    作者: 可确认的实施者或来源
    说明: 本次变更的边界与关键决策
  ```
- 维护注记的归属只能来自可核验的 Git 记录、提交/PR 元数据或用户明确确认；无法确认时明确标注未确认，不得根据文风、工具痕迹或当前修改者猜测
- 后续修改既有注记时保留原日期与归属；新的实质变更另作记录，不把多次变更统一归给最后修改者
