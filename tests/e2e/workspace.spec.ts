...
test.describe('workspace browser acceptance', () => {
  ...
  test('opens debug page and keeps invalid JSON inside the tool form', async ({ page }) => {
    ...
    await select.selectOption('route_planner')
    const jsonField = page.getByRole('textbox', { name: 'waypoints' })
  ...
  test('supports mobile panel tabs and multiline composer without horizontal overflow', async ({ page }) => {
    ...
    await expect(content).toHaveAttribute('data-mobile-panel', 'map')
    await expect(page.getByRole('region', { name: '空间地图' })).toBeVisible()
    await mobileTabs.getByRole('button', { name: '结果' }).click()
    await expect(content).toHaveAttribute('data-mobile-panel', 'results')
    await expect(page.locator('.workbench-pane--inspector')).toBeVisible()
    await mobileTabs.getByRole('button', { name: '工具' }).click()
    await expect(content).toHaveAttribute('data-mobile-panel', 'tools')
    await expect(page.locator('.workbench-pane--tools')).toBeVisible()
    await mobileTabs.getByRole('button', { name: '对话' }).click()
    await expect(content).toHaveAttribute('data-mobile-panel', 'chat')
    await expect(composer).toBeVisible()
  ...
  test('opens ArcGIS-style layer manager from the inspector layer action', async ({ page }) => {
    const errors = collectUnexpectedErrors(page)
    await page.goto('/')
    await page.getByRole('region', { name: '空间地图' }).getByRole('button', { name: '图层管理', exact: true }).click()

    const panel = page.getByRole('region', { name: '图层管理' })
    await expect(panel).toBeVisible()
    await expect(panel.getByRole('heading', { name: '图层管理' })).toBeVisible()
    await expect(panel.getByRole('textbox', { name: '搜索图层' })).toBeVisible()
    await expect(panel.getByRole('button', { name: '绘制顺序' })).toBeVisible()
    await expect(panel.getByRole('tree', { name: '地图图层树' })).toBeVisible()
    await expect(panel.getByText('暂无可管理图层')).toBeVisible()
    await expect(panel.getByText('底图仍由地图画布单独管理', { exact: false })).toBeVisible()
    await expect(page.getByText('页面遇到问题')).toHaveCount(0)
    expect(errors).toEqual([])
  })
