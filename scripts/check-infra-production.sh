...
docker compose -f "$COMPOSE" exec -T postgis psql -U "${POSTGRES_USER:-geo_agent}" -d "${POSTGRES_DB:-geo_agent}" -tAc "SELECT to_regclass('public.platform_terminal_sessions') IS NOT NULL" | grep -qx t
