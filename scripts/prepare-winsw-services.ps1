...
param(
    ...
    [string]$ProcessComposeEnvironmentFile = (Join-Path $env:ProgramData 'GeoForge\process-compose.env'),
    [string]$OpsGatewayEnvironmentFile = (Join-Path $env:ProgramData 'GeoForge\ops-gateway.env'),
    [string]$TerminalBrokerEnvironmentFile = (Join-Path $env:ProgramData 'GeoForge\terminal-broker.env')
)
...
$Services = @(
    @{ Name = 'GeoForgeProcessCompose'; Template = 'GeoForgeProcessCompose.xml.template'; Environment = $ProcessComposeEnvironmentFile },
    @{ Name = 'GeoForgeOpsGateway'; Template = 'GeoForgeOpsGateway.xml.template'; Environment = $OpsGatewayEnvironmentFile },
    @{ Name = 'GeoForgeTerminalBroker'; Template = 'GeoForgeTerminalBroker.xml.template'; Environment = $TerminalBrokerEnvironmentFile }
)
