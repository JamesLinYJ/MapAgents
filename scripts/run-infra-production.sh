...
docker compose -f "$COMPOSE" exec -T postgis psql -U "${POSTGRES_USER:-geo_agent}" -d "${POSTGRES_DB:-geo_agent}" -v ON_ERROR_STOP=1 -f /docker-entrypoint-initdb.d/003_operations_terminal.sql
