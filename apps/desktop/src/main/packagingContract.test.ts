...
describe('desktop packaging contract', () => {
  it('aligns the Desktop release version and builds internal workspaces from a cold checkout', async () => {
    ...
    expect(desktopPackage.version).toBe('0.1.3')
  ...
  it('keeps Windows and Linux package identity and metadata explicit in Forge', async () => {
    ...
    expect(rpmSpecSource).toContain('resources/runtime-service/deploy/bin/geoforge')
    expect(rpmSpecSource).toContain('/usr/bin/geoforge')
