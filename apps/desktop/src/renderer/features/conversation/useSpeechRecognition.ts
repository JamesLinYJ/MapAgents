...
export function useSpeechRecognition({
  ...
  const startRecognition = useCallback(async () => {
    ...
    try {
      const authorization = await requestSpeechRecognitionAuthorization()
...
export async function requestSpeechRecognitionAuthorization(
  dependencies: {
    requestMicrophonePermission(): Promise<{ granted: boolean; message: string | null }>
    requestSpeechAuthorization(): Promise<SpeechAuthorization>
  } = {
    requestMicrophonePermission: () => (
      requireDesktopBridge().permissions.requestSpeechMicrophone()
    ),
    requestSpeechAuthorization: getSpeechAuthorization,
  },
): Promise<SpeechAuthorization> {
  const permission = await dependencies.requestMicrophonePermission()
  if (!permission.granted) {
    throw new Error(
      permission.message ?? '未获得本次麦克风授权，语音识别未启动。',
    )
  }
  return dependencies.requestSpeechAuthorization()
}
