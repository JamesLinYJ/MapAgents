...
export function ToolMiniAppResult({ toolName, result, artifacts, onSelectArtifact }: ToolMiniAppResultProps) {
  ...
  const downloadableArtifacts = artifacts.filter((artifact) => (
    Boolean(artifact.uri?.trim()) && artifactHasDisplaySurface(artifact, 'download')
  ))
  ...
  const downloadArtifact = async (artifact: MiniArtifact) => {
    const artifactKey = artifact.artifactId ?? artifact.uri ?? ''
    if (!artifactKey || downloadingArtifactKey) return
  ...
  return (
    <section className={`tool-mini-result tool-mini-result--${kind}`}>
        ...
        <div className="tool-mini-result__downloads">
          {downloadableArtifacts.map((artifact) => (
            <button
              key={artifact.artifactId ?? artifact.uri}
              type="button"
              disabled={downloadingArtifactKey !== null}
              onClick={() => {
                void downloadArtifact(artifact)
              }}
            >
              <Download size={14} aria-hidden="true" />
              {downloadingArtifactKey === (artifact.artifactId ?? artifact.uri)
                ? '正在保存…'
      ...
      {downloadMessage ? (
        <p className="tool-mini-result__download-status" role="status">{downloadMessage}</p>
      ) : null}
    </section>
  )
}
