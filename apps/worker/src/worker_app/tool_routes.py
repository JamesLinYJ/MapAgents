...
from worker_app.tool_registry import dispatch
...
def register_tool_routes(app: FastAPI, *, tool_timeout_seconds: float, logger: logging.Logger) -> None:
    ...
    async def run_meteorology_tool(tool_name: str, tool_request: ToolRequest, request: Request) -> dict[str, Any]:
        ...
        try:
            payload = await asyncio.wait_for(
                asyncio.to_thread(dispatch, tool_name, tool_request.args),
