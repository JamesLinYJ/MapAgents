          ...
          <section>
            ...
            <div className="automation-definition-list">
              {filteredDefinitions.map(definition => (
                <button type="button" className={definition.automationId === effectiveSelectedAutomationId ? 'is-active' : ''} key={definition.automationId} onClick={() => { if (dirty && !window.confirm('当前更改尚未保存，确定切换吗？')) return; setDirty(false); setSelectedAutomationId(definition.automationId); setDraft(draftFromDefinition(definition)); setValidationResult(validation[definition.automationId] ?? null); setSelectedNodeId(null); setCanvasDocumentVersion(current => current + 1) }}>
    ...
    <article className="automation-run-card">
      ...
      {artifacts.length ? (
        <section className="automation-run-card__artifacts" aria-label="可核验交付物">
          ...
          <div>{artifacts.map(artifact => <a key={artifact.artifactId} href={artifact.uri} target="_blank" rel="noreferrer"><Download size={14} /><strong>{artifact.name}</strong><small>{artifact.artifactType}</small></a>)}</div>
        </section>
      ) : null}
...
async function importDraft(
  ...
  try {
    const parsed = automationDefinitionSchema.parse(JSON.parse(await file.text()))
