...
const TERMS_SECTIONS = [
  {
    title: '1. 服务范围',
    body: [
      'GeoForge 提供气象数据分析、地图浏览、图层管理、智能体工具调用、Workflow、定时任务、语音输入、报告和 Artifact 生成等工作台能力。',
  ...
  },
  {
    title: '2. 账号与工作区',
    body: [
      '用户通过 GeoForge 账号登录后访问所属工作区。账号、成员关系、角色和权限以服务端 Better Auth 与 Casbin 校验结果为准。',
  ...
  },
  {
    ...
    body: [
      ...
      '平台生成的图层、统计、报告、音频、图片和其他 Artifact 会记录在对应工作区和运行历史中，并受到相同权限边界保护。',
    ],
  },
  {
    title: '4. 智能体、工具与审批',
    body: [
      ...
      '计划模式、审批模式、Workflow 和定时任务不会绕过服务端权限；任何权限不足的操作都会失败并记录审计。',
    ],
  },
  {
    title: '5. 语音与第三方服务',
  ...
  },
  {
    title: '7. 服务变更与终止',
    body: [
      '平台可能根据安全、稳定性或产品演进调整工具、模型、Workflow、配额和权限策略。',
  ...
  },
  {
    title: '8. 责任限制',
    body: [
      'GeoForge 尽力提供可靠的数据处理链路、日志、指标和审计，但不保证第三方模型、外部 API、上传数据或科学算法在所有场景下无误。',
...
const PRIVACY_SECTIONS = [
  {
    ...
    body: [
      ...
      '业务数据：用户输入、上传文件、气象数据集、图层、Artifact、Workflow 参数、定时任务配置、工具调用记录和运行历史。',
      '安全数据：CSRF token、会话状态、审计日志、限流记录、错误日志、Worker 调用状态和必要的诊断指标。',
    ],
  },
  {
    title: '2. 使用目的',
    body: [
      ...
      '用于记录审计事件、检测滥用、保护系统安全、统计真实 token 用量和改进平台可靠性。',
    ],
  },
  {
    title: '3. 模型与工具处理',
    body: [
      ...
      '历史对话不会被运行时静默注入 prompt；需要历史事实时，智能体必须通过上下文工具显式读取。',
    ],
  },
  {
    title: '4. 存储与隔离',
    body: [
      '会话历史以文件型 conversation store 保存；用户、工作区、权限、图层和索引元数据保存在 Postgres/PostGIS。',
      '私有记忆绑定用户，团队记忆绑定工作区，运行时 Artifact 绑定 thread/run/workspace。资源 ID 不是访问令牌，每次访问都由服务端回校权限。',
    ],
  },
  {
    title: '5. Cookie 与短期授权',
    body: [
      'GeoForge 使用 HTTP-only Cookie 保存 Better Auth 登录会话，并使用 CSRF token 保护可变更请求。',
      'Azure Speech 等浏览器侧能力只接收短期授权 token，不接收长期订阅 key；前端不会将长期密钥写入 localStorage、运行历史或工具结果。',
    ],
  },
  {
    title: '6. 共享与第三方',
    body: [
      ...
      '当工作区管理员导出 Artifact、报告或图层时，导出文件的后续使用由导出者和所属组织负责。',
    ],
  },
  {
    title: '7. 保留与删除',
    body: [
      '会话、运行、Artifact、审计和日志的保留周期由部署环境和工作区策略决定。删除或归档操作应通过服务端接口完成，并记录必要审计。',
...
export function LegalPolicyPage({ kind }: LegalPolicyPageProps) {
  ...
  const title = isTerms ? 'GeoForge 服务协议' : 'GeoForge 隐私政策'
    ...
    <main className="legal-page" aria-labelledby="legal-title">
      <article className="legal-shell">
        <header className="legal-hero">
          ...
          <p>
            最近更新：2026年7月9日。本文面向 GeoForge 工作台使用场景，覆盖账号、权限、气象数据、
            模型工具、Workflow、语音服务、审计和运行历史等能力。
          </p>
