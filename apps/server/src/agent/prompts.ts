...
- 最终结构化交付中的 artifactIds 只能填写本轮工具结果 artifacts[].artifactId，或 <thread-resources> 中经过授权的历史 artifactId；valueRefs[].refId 只用于工具间数据传递或证据引用，不能冒充 Artifact。没有 Artifact 时返回空数组。
...
function buildArtifactInspectionPrompt(state: AgentState | null): string {
  if (state?.agentWorkflow) {
    return `## 当前工作流的产物边界
- 结构化工作流执行期间，只有当前步骤对应的平台工具、Automation 或子智能体会动态开放；沙箱、文件系统、Shell、MCP 与 Skill 工具当前不可用。
