...
const BASE_MEMORY_RULES = [
  '记忆工具只用于长期、可复用、不可从当前仓库或运行事实源直接推导的信息。',
  ...
  '当前工具注册表、执行能力目录、Automation 清单和参数 Schema 是能力事实源；不要搜索、读取或保存记忆来学习或确认工具/Automation 的名称、参数、默认值、示例和可用性。',
...
export const SEARCH_MEMORY_PROMPT = [
  ...
  '',
  '用于按用户问题检索相关长期记忆。',
  '当用户说“记得/之前/上次/回忆/按我的偏好/忘记什么”等，或当前任务明确依赖当前线程中不存在的长期偏好、历史决策、团队约定或外部引用时使用。',
  '不要仅因为记忆索引中出现了与当前工具或 Automation 相似的名称就搜索；当前能力契约必须直接读取平台事实源。',
