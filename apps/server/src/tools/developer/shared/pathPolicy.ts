...
import {
  assertDeveloperPathNotSensitive,
  type SecretPathConfig,
} from './secretPathPolicy.js'
...
export async function resolveDeveloperPath(
  ...
  assertDeveloperPathNotSensitive(absolutePath, options.secretConfig ?? process.env)
  ...
  if (options.mustExist) {
    ...
    assertDeveloperPathNotSensitive(realTarget, options.secretConfig ?? process.env)
