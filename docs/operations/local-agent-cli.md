...
地理智能平台 本机 Agent CLI 是面向已登录服务器操作系统的人工交互入口。它复用主 API 中唯一的 Agent Runner、RunState、工具审批、工作流、审计和 PostgreSQL 会话事实源；CLI 本身不创建第二套 Agent 运行时，也不直接调用模型。
...
- 顶部使用 地理智能平台 深蓝灰主题、青色主焦点、蓝色模型、紫色思考、绿色成功、琥珀色等待和红色失败；状态始终同时显示符号与中文标签，不只依赖颜色；
...
这套授权只允许调用已经注册、带 Zod 契约和 RBAC policy 的 地理智能平台 WS 命令。它不是 Shell，不接入监督器任意命令，不注册为 Agent Tool 或 Automation 动作。
