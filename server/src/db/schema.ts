...
export const platformTerminalSessions = pgTable('platform_terminal_sessions', {
  terminalId: text('terminal_id').primaryKey(),
  ownerUserId: text('owner_user_id').notNull().references(() => platformUsers.userId, { onDelete: 'restrict' }),
  label: text('label').notNull(),
  state: text('state').notNull(),
  shell: text('shell').notNull(),
  initialCols: integer('initial_cols').notNull(),
  initialRows: integer('initial_rows').notNull(),
  currentCols: integer('current_cols').notNull(),
  currentRows: integer('current_rows').notNull(),
  pid: integer('pid'),
  ...
  keyWrapNonce: text('key_wrap_nonce').notNull(),
  keyWrapAuthTag: text('key_wrap_auth_tag').notNull(),
  recordedBytes: integer('recorded_bytes').notNull().default(0),
  lastChunkSequence: integer('last_chunk_sequence').notNull().default(-1),
  createdAt: timestamp('created_at', { withTimezone: true }).notNull().defaultNow(),
  ...
  ownerStateIdx: index('idx_terminal_sessions_owner_state').on(table.ownerUserId, table.state),
  retentionIdx: index('idx_terminal_sessions_retention').on(table.retainedUntil),
  stateCheck: check('platform_terminal_sessions_state_check', sql`${table.state} IN ('starting', 'running', 'detached', 'exited', 'terminated', 'failed', 'orphaned')`),
  colsCheck: check('platform_terminal_sessions_cols_check', sql`${table.currentCols} BETWEEN 20 AND 400 AND ${table.initialCols} BETWEEN 20 AND 400`),
  rowsCheck: check('platform_terminal_sessions_rows_check', sql`${table.currentRows} BETWEEN 5 AND 200 AND ${table.initialRows} BETWEEN 5 AND 200`),
  recordedBytesCheck: check('platform_terminal_sessions_recorded_bytes_check', sql`${table.recordedBytes} >= 0 AND ${table.recordedBytes} <= 536870912`),
}))

export const platformTerminalTranscriptChunks = pgTable('platform_terminal_transcript_chunks', {
  terminalId: text('terminal_id').notNull().references(() => platformTerminalSessions.terminalId, { onDelete: 'cascade' }),
  sequence: integer('sequence').notNull(),
  contentHash: text('content_hash').notNull(),
  sizeBytes: integer('size_bytes').notNull(),
  eventCount: integer('event_count').notNull(),
  firstEventMilliseconds: integer('first_event_milliseconds').notNull(),
  lastEventMilliseconds: integer('last_event_milliseconds').notNull(),
  createdAt: timestamp('created_at', { withTimezone: true }).notNull().defaultNow(),
}, (table) => ({
  pk: primaryKey({ columns: [table.terminalId, table.sequence], name: 'platform_terminal_transcript_chunks_pk' }),
  contentHashIdx: index('idx_terminal_transcript_chunks_content_hash').on(table.contentHash),
  sizeCheck: check('platform_terminal_transcript_chunks_size_check', sql`${table.sizeBytes} > 0`),
  sequenceCheck: check('platform_terminal_transcript_chunks_sequence_check', sql`${table.sequence} >= 0`),
}))

export const platformTerminalAccessGrants = pgTable('platform_terminal_access_grants', {
  grantId: text('grant_id').primaryKey(),
  terminalId: text('terminal_id').notNull().references(() => platformTerminalSessions.terminalId, { onDelete: 'cascade' }),
  grantedToUserId: text('granted_to_user_id').notNull().references(() => platformUsers.userId, { onDelete: 'cascade' }),
  reason: text('reason').notNull(),
  expiresAt: timestamp('expires_at', { withTimezone: true }).notNull(),
  usedAt: timestamp('used_at', { withTimezone: true }),
  createdAt: timestamp('created_at', { withTimezone: true }).notNull().defaultNow(),
}, (table) => ({
  granteeExpiryIdx: index('idx_terminal_access_grants_grantee_expiry').on(table.grantedToUserId, table.expiresAt),
  terminalCreatedIdx: index('idx_terminal_access_grants_terminal_created').on(table.terminalId, table.createdAt),
}))
