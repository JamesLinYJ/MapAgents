...
import type { WorkflowBinding } from './schemas.js'
...
export function resolveWorkflowBinding(binding: WorkflowBinding, context: WorkflowBindingContext): unknown {
  if (binding.source === 'literal') return structuredClone(binding.value)
  if (binding.source === 'input') {
    return resolvePath({ prompt: context.prompt, parameters: context.parameters }, binding.path, 'Workflow 输入')
  }
...
export function renderWorkflowPrompt(template: string, context: WorkflowBindingContext): string {
